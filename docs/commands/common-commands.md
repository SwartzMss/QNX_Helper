...
```text
sh: sloginfo: cannot execute - No such file or directory
```
...
```text
sh: showmem: cannot execute - No such file or directory
```

这说明在当前 Raspberry Pi 5 的 QNX 环境中，这两个命令暂时没有直接可用，后续如果安装了对应组件或发现替代命令，可以再补充。

## 7. 当前阶段建议优先掌握的命令

如果目前只是做学习和基础调试，建议先把下面这些用熟：
